...
(() => {
  const course = document.querySelector('.course-title .long-name')?.textContent;
  const sideWrap = document.querySelector('.side-wrap');
  const cards = sideWrap.querySelectorAll('.card');
  ...
  fetch('http://localhost:3002/api/flashcards/vocab', { method: 'POST',  mode: 'cors', headers: { 'Content-Type': 'application/json' }, body: vocabJSON }).then(() => { console.log('save done') }).catch(() => { console.log('save fail') });
})()
...
const vocabWithApi = () => {
  ...
  const vocabJSON = JSON.stringify(vocab, null, 2);
  fetch('https://jp-api.vercel.app/api/flashcards/vocab', { method: 'POST',  mode: 'cors', headers: { 'Content-Type': 'application/json' }, body: vocabJSON }).then(() => { console.log('save done') }).catch(() => { console.log('save fail') });
...
// nutrition quiz
javascript:(() => { const quiz = document.querySelector('.quiz-review'); const cards = quiz.querySelectorAll('.card'); const chapter = document.querySelector('.page-header .main')?.textContent; const title = document.querySelector('.page-header .sub')?.textContent; const questions = Array.from(cards).map((cardEl) => { const questionMarkdown = cardEl.querySelector('.card-header markdown'); const question = questionMarkdown.textContent || questionMarkdown.innerText || ''; const optionsMarkdown = cardEl.querySelectorAll('.card-body markdown'); const options = Array.from(optionsMarkdown).map((el) => el.textContent || el.innerText || ''); const answerEl = cardEl.querySelector('.text-success'); const answer = answerEl.textContent || answerEl.innerText || ''; const wrongAnswer = cardEl.querySelector('.text-danger'); return { question, options, answer, missed: !!wrongAnswer, tags: ['nutrition', 'quiz', chapter], title, chapter }; }); const questionsJSON = JSON.stringify(questions, null, 2); fetch('https://jp-api.vercel.app/api/flashcards/quiz', { method: 'PUT', mode: 'cors', headers: { 'Content-Type': 'application/json' }, body: questionsJSON }).then(() => { alert('success') }).catch(() => { alert('fail') }); })()
